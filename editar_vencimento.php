...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE vencimentos SET validade=?, documento=?, empresa=?, responsavel=?, situacao=?, proxima_data=?, observacao=? WHERE id=?";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssi", $validade, $documento, $empresa, $responsavel, $situacao, $proxima_data, $observacao, $id);
        ...
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    <h4 class="text-lg font-semibold text-gray-800">Editar Vencimento</h4>
                ...
                <div class="p-6">
                    ...
                    <form action="" method="POST" class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="validade" class="block text-sm font-medium text-gray-700 mb-1">Data de Validade</label>
                                <input type="date" class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" id="validade" name="validade" value="<?php echo $vencimento['validade']; ?>" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow bg-white" id="situacao" name="situacao" required>
                                    <option value="Em dia" <?php echo ($vencimento['situacao'] == 'Em dia') ? 'selected' : ''; ?>>Em dia</option>
                                    <option value="Pendente" <?php echo ($vencimento['situacao'] == 'Pendente') ? 'selected' : ''; ?>>Pendente</option>
                                    <option value="Vencido" <?php echo ($vencimento['situacao'] == 'Vencido') ? 'selected' : ''; ?>>Vencido</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" id="documento" name="documento" value="<?php echo htmlspecialchars($vencimento['documento']); ?>" required>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="empresa" class="block text-sm font-medium text-gray-700 mb-1">Empresa</label>
                                <input type="text" class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" id="empresa" name="empresa" value="<?php echo htmlspecialchars($vencimento['empresa']); ?>" required>
                            </div>
                            
                            <div>
                                <label for="responsavel" class="block text-sm font-medium text-gray-700 mb-1">Responsável</label>
                                <input type="text" class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" id="responsavel" name="responsavel" value="<?php echo htmlspecialchars($vencimento['responsavel']); ?>" required>
                            </div>
                        </div>

                        <div>
                            <label for="proxima_data" class="block text-sm font-medium text-gray-700 mb-1">Próxima Data (Opcional)</label>
                            <input type="date" class="w-full rounded-lg border-gray-300 border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-shadow" id="proxima_data" name="proxima_data" value="<?php echo $vencimento['proxima_data']; ?>">
                            <p class="text-xs text-gray-500 mt-1">Data prevista para a próxima renovação.</p>
                        </div>

                        <!-- Campo de Observação (visível apenas quando Pendente) -->
                        <div id="campo_observacao" style="display: <?php echo ($vencimento['situacao'] == 'Pendente') ? 'block' : 'none'; ?>;">
                            ...
                            <textarea 
                                ...
                                name="observacao" 
                                rows="3"
                                placeholder="Descreva o motivo da pendência..."><?php echo isset($vencimento['observacao']) ? htmlspecialchars($vencimento['observacao']) : ''; ?></textarea>
